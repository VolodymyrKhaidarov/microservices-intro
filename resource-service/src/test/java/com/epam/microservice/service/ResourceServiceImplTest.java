...
class ResourceServiceImplTest {
  ...
  void addResourceTest() {
    ...
    ResourceService resourceService = new ResourceServiceImpl(resourceRepository, s3Service, kafkaTemplate);
    ...
    Mockito.when(s3Service.getS3Bucket()).thenReturn(S3BUCKET);
    Mockito.when(s3Service.uploadResource(multipartFile)).thenReturn(FILENAME);
  ...
  void getResourceTest() {
    ...
    Mockito.when(s3Service.downloadResource(FILENAME)).thenReturn(Optional.of(PAYLOAD));
